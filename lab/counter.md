...
Create a new folder in `src/app/areas` called `counter-redo`.
...
In the `src/app/area/counter-redo` folder, create a file called `counter.routes.ts`.
...
Use a `canMatch` guard that only uses this version if the `counter-lab` feature is on.
...
```
  {
    ...
    loadChildren: () =>import('./area/counter-redo/counter.routes').then(r => r.COUNTER_ROUTES)
  },
```

> **NOTE**: Add this _before_ the existing route for the counter.

In the file `src/mocks/feature-handler.ts`, add `counter-lab` to the array of features to be enabled. (line 3).
...
1. In the `src/app/areas/counter-redo` folder create a new directory called `pages` and in that folder, create a new component called `ui.component.ts`.
